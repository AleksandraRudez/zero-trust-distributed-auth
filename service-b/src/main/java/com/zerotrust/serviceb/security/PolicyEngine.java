...
public class PolicyEngine {
    ...
    private final Set<String> allowedInternalCallers = new java.util.concurrent.CopyOnWriteArraySet<>(Set.of("service-c"));
    ...
    public boolean evaluatePolicy(Context context) {
        ...
        // SERVICE može pristupiti /api/internal i /api/orchestrate
        if ("SERVICE".equalsIgnoreCase(context.getRole())) {
            if (!context.getRequestPath().startsWith("/api/internal") && !context.getRequestPath().startsWith("/api/orchestrate")) {
                context.setDenialReason("Servisi mogu pristupiti samo /api/internal ili /api/orchestrate");
                log.warn("DENY: Service {} cannot access {}", context.getUserId(), context.getRequestPath());
                return false;
            }

            // Policy enforcement: pristup dozvoljen samo određenim servisima
            ...
            log.info("ALLOW: Service {} can access {}", context.getUserId(), context.getRequestPath());
            return true;
        }
